...
```bash
npm test              # main Aegis suite — 434 tests
...
Two files are **deliberately excluded** from `npm test`:
`src/risk/__tests__/anthropic-judge.live.test.ts` and
`src/rails/__tests__/stripe-rail.live.test.ts` — reachable only via
`test:risk:live` / `test:rails:live`, and only if a real `ANTHROPIC_API_KEY` /
`STRIPE_SECRET_KEY` is present. They are not part of the standard, credential-free run.
...
492 tests total, 0 failures, reproduced immediately before this document was written.
...
| Risk | `src/risk/__tests__/risk-core.test.ts` | The behavioral-baseline heuristics in isolation: rate-window boundaries, the amount-deviation multiplier, and that transactions outside the rate window correctly don't count |
...
| Demo mode | `src/api/__tests__/demo-mode.test.ts`, `demo-tamper.test.ts` | `AEGIS_DEMO_MODE`'s exact, narrow surface (which judge, which rails) and the demo-only tamper route used by the Evidence workspace |
...
- **No real AI risk evaluation is exercised.** `anthropic-judge.live.test.ts` requires a
  real `ANTHROPIC_API_KEY` and is excluded from `npm test` — the standard run never
  calls a real model.
